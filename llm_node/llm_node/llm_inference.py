...
- 모드: START | DELIVER | RETURN | FINISH | INFO | LIGHT_ON | LIGHT_OFF
- INFO: 도구 질의
- LIGHT_ON / LIGHT_OFF: 조명 제어
- parse_and_infer가 {"mode", "tool_final", "section"} 3개 키만 반환
- section 미언급 시 기본값 0  (LIGHT_ON에서 0이면 앱에서 1번 섹션으로 보정)
...
class ToolChatEngine:
    # 7개 모드
    CANDIDATES = {"START", "DELIVER", "RETURN", "FINISH", "INFO", "LIGHT_ON", "LIGHT_OFF"}
    _MODE_RE = re.compile(r"\b(START|DELIVER|RETURN|FINISH|INFO|LIGHT_ON|LIGHT_OFF)\b", re.IGNORECASE)
    ...
    - LIGHT_ON: 조명 비추기 요청. "3번 구역 비춰줘", "1섹션에 불 켜", "
    - LIGHT_OFF: 조명 끄기 요청. "불 꺼줘", "조명 꺼"
    ...
    def _setup_mode_rag(self, scenario_text: str, jsonl_path: Optional[str]):
        ...
        mode_template = (
            ...
            "가능한 모드: START | DELIVER | RETURN | FINISH | INFO | LIGHT_ON | LIGHT_OFF\n"
    ...
    def _rule_based_mode(self, clause: str) -> Optional[str]:
        ...
        if any(k in t for k in ["비춰", "비추", "조명", "불 켜", "불켜", "스탠드 켜"]):
            return "LIGHT_ON"

        if any(k in t for k in ["불 꺼", "불꺼", "조명 꺼", "스탠드 꺼"]):
            return "LIGHT_OFF"

        # 섹션/구역 언급 → LIGHT_ON 기본 후보
    ...
    def llm_extract_tool_and_target(self, user_text: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        """
        tool_candidates = ["M3 나사", "니퍼", "커터", "와이어 스트리퍼", "NONE"]
    ...
    def parse_and_infer(self, user_text: str) -> Dict[str, Any]:
        ...
        - 모드: 마지막 문장만 기준으로 결정
        - 도구: 전체 문장을 기준으로 추론
        - 섹션: 없으면 0 (LIGHT_ON에서 0이면 앱에서 1번 섹션으로 보정)
        """
        ...
        if not mode:
            system_instructions = (
                ...
                "이 문장을 읽고 7모드(START, DELIVER, RETURN, FINISH, INFO, LIGHT_ON, LIGHT_OFF) 중 하나로 분류하라.\n"
        ...
        # 3. section이 있으면 무조건 LIGHT_ON으로 override
    ...
    def generate_speech(self, task_description: str, style: str = "친절하고 상냥하게") -> str:
        ...
        - (작업: LIGHT_ON, 구역: 3) -> "네, 3번 구역에 조명을 비추겠습니다."
        - (작업: LIGHT_ON, 구역: 1) -> "네, 1번 구역을 비춰드릴게요."
        - (작업: LIGHT_OFF) -> "네, 조명을 끄겠습니다."

        - (작업: 이미 다른 작업 진행 중) -> "현재 진행 중인 작업이 있어서, 이 작업이 끝난 뒤에 새 작업을 도와드릴게요."
